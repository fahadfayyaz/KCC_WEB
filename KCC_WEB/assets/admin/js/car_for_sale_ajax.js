    ...
    $(document).ready(function($) {
        ...
        $("#activeAllStatus").on('click', function(event) {
        	...
        	if ($(".chkParam:checked").length > 0 ) 
        	{
        		...
        		$.post('/admin/car_for_sale/active_all_status', formSerials, function(response) {
        			if (response > 0) 
        				window.location.href =  "/admin/car_for_sale";
        		});
        	}
        	else
        ...
        $("#deactiveAllStatus").on('click', function(event) {
        	...
        	{
        		var formSerials = $("#formViewAjax").serialize();
        		$.post('/admin/car_for_sale/deactive_all_status', formSerials, function(response) {
        			if (response > 0) 
        				window.location.href =  "/admin/car_for_sale";
        		});
        	}
        ...
        $("#deleteAll").on('click', function(event) {
        	...
        	if ($(".chkParam:checked").length > 0 ) 
        	{
        		...
        		{
	        		var formSerials = $("#formViewAjax").serialize();
	        		$.post('/admin/car_for_sale/delete_all', formSerials, function(response) {
	        			if (response > 0) 
	        				window.location.href =  "/admin/car_for_sale";
	        		});
        		}
